...
@section('header_subtitle', 'Sunting kontent publikasi yang sudah ada.')
            ...
            <div class="bg-white rounded-xl shadow-card border border-slate-100 p-6">
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $post->title) }}" required class="block w-full border border-slate-200 rounded-lg focus:ring-2 focus:ring-riau-green focus:border-riau-green sm:text-sm text-slate-900 bg-slate-50 py-2.5 px-3">
                ...
                <div class="space-y-5">
                    <div>
                        ...
                        <select name="category" id="category" class="block w-full border border-slate-200 rounded-lg focus:ring-2 focus:ring-riau-green focus:border-riau-green sm:text-sm text-slate-900 bg-slate-50 py-2.5 px-3">
                            <option value="news" {{ old('category', $post->category) == 'news' ? 'selected' : '' }}>Berita Aktual</option>
                            <option value="article" {{ old('category', $post->category) == 'article' ? 'selected' : '' }}>Artikel Edukasi</option>
                        </select>
                    ...
                    <div>
                        ...
                        @if($post->image)
                            <div class="mb-3 rounded-lg overflow-hidden border border-slate-200 aspect-video flex-shrink-0 bg-slate-100">
                                <img src="{{ $post->image_url }}" alt="Thumbnail Saat ini" class="w-full h-full object-cover">
                            </div>
                        @endif
                        <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300 border-dashed rounded-lg hover:border-riau-green transition-colors cursor-pointer bg-slate-50 relative group">
                            <div class="space-y-1 text-center">
                                ...
                                <p class="text-xs text-slate-500">Biarkan kosong jika tak ingin ganti.</p>
                            </div>
                        </div>
                    ...
                    <div class="pt-4 border-t border-slate-100">
                        <div class="flex items-center">
                            <input id="is_published" name="is_published" type="checkbox" value="1" {{ old('is_published', $post->is_published) ? 'checked' : '' }} class="h-4 w-4 text-riau-green focus:ring-riau-green border-slate-300 rounded">
